        ...
        >
          ...
          >
            <div class="w-48">
              ...
              <div class="text-xs text-gray-500 mb-2">Drag pin to locate spot</div>
              <input
                v-model="newSpotName"
          ...
          >
            <div class="text-center">
              ...
              <div class="text-xs text-gray-600">
                {{ formatCoordinates(createModal.lat, createModal.lng) }}
              </div>
            </div>
          </l-tooltip>
        </l-marker>
...
<script setup lang="ts">
  ...
  watch(center, (newCenter, oldCenter) => {
    if (mapRef.value && newCenter !== oldCenter) {
      ...
      const distance =
        Math.abs(newCenter[0] - oldCenter[0]) + Math.abs(newCenter[1] - oldCenter[1]);
      if (distance > 0.001) {
        // Only update if moved more than ~100m
  ...
  const handleCreateSpotFromModal = async (payload: {
    name: string;
    description: string;
    lat: number;
    lng: number;
